...
def SGDrandom(x, y, L_size):
    ...
    for i in range(len(L_size)):
        if i == 0:
            W = np.random.normal(0, 1, (L_size[i], x.shape[0]))
            b = np.zeros((L_size[i], 1))
        else:
            W = np.random.normal(0, 1, (L_size[i], L_size[i - 1]))
            b = np.zeros((L_size[i], 1))
        w_set['W' + str(i + 1)] = W
        w_set['b' + str(i + 1)] = b
    W = np.random.normal(0, 1, (dim_y, L_size[-1]))
    b = np.zeros((dim_y, 1))
    ...
    for t in range(0,50):
        ...
        for i in range(train_x.shape[1]):
            ...
            if i == 1 and t == 4 and L_size == [5,5]:
                print('we select a training example randomly.')
                print('when x = ', x[:, i], ', y = ', y[:, i], ', width is ', L_size[0])
                print('the gradient is ', grad_weights)
...
def forward_pass(x, w_set):
    ...
    # print('len(weights) // 2', len(weights) // 2)
    i = 0
...
m = 1
print('when the initial weights are random')
for w in widths:
    w_set = SGDrandom(train_x, train_y, L_size=[w, w])
