...
def value_final(final_Label):
    ...
    return finalLabel_key, finalLabel_value
...
def extract_label(test_attribut, finalLabel_key, test_label, finalLabel_value):
    compare_Label = []
    labelPair = []
    b = 0
    for every in test_attribut:
        ...
        if b == 1:
            ...
            compare_Label.append(copy.deepcopy(labelPair))
            compare_test.append(copy.deepcopy(test_label[test_attribut.index(every)]))
            compare_train.append('0')
            # print('compare label2 is ', compare_Label)
            b = 0
    return compare_Label, compare_test, compare_train

def labelTrans(compare_train, compare_test):
    for i in range (0,len(compare_test)):
        if compare_train[i] == '0':
            compare_train[i] = -1
        else:
            compare_train[i] = 1
        ...
        if compare_test[i] == '0':
            ...
            compare_test[i] =1
    return compare_train, compare_test

def weight_error(Value_train, Value_test, weight):
    error = 0.0
    errornum = 0
    for i in range (0,len(Value_test)):
        if Value_train[i] != Value_test[i]:
            error += weight[i]
            errornum +=1
    return error, errornum

# calculate the new weights
def new_weight(Value_train, Value_test, weight, alpha):
    newweightset = []
    normweight=[]
    norm = 0.0
    sum = 0.0
    for i in range(0, len(Value_test)):
        if Value_train[i] ==Value_test[i]:
            WeightNew = weight[i]*math.exp(-alpha)
        else:
            WeightNew = weight[i]*math.exp(alpha)
        newweightset.append(WeightNew)
        norm += WeightNew
    for each in newweightset:
        normvalue = each/norm
        normweight.append(normvalue)
        sum+=normvalue
    print('the new weight sum is ', sum)
    return normweight


def bgd(Value_train, Value_test, weight, hypothesis):
    error_list = [1]
    alphaset = []
    for t in range(0, 500):

        error, errornum = weight_error(Value_train, Value_test, weight)
        print('error:', error)
        # calculate alpha
        alpha = 0.5 * math.log((1 - error) / error)
        print('alpha', alpha)
        alphaset.append(copy.deepcopy(alpha))
        weight = new_weight(Value_train, Value_test, weight, alpha)
        subhypoth = [i * alpha for i in Value_train]
        hypothesis = np.sum([subhypoth, hypothesis], axis=0).tolist()
        Hfinal = np.sign(hypothesis)
        Hfinal_list = Hfinal.tolist()
        q = 0
        newerror = 0.0
        for i in range(0, len(Value_test)):
            if Hfinal_list[i] != Value_test[i]:
                q += 1
        newerror = q / len(Value_test)
        print('error is ', newerror)
        error_list.append(copy.deepcopy(newerror))
...
finalLabel_key, finalLabel_value=value_final(final_Label)
print(value_final(final_Label))
print(final_Label)
...
print('L', test_label)
compare_Label, compare_test, compare_train=extract_label(test_attribut, finalLabel_key, test_label, finalLabel_value)
Value_train, Value_test = labelTrans(compare_train, compare_test)

hypothesis=[0 for _ in range(len(compare_test))]
weight = [1/len(compare_test) for _ in range(len(compare_test))]

error_list = bgd(Value_train, Value_test, weight, hypothesis)

####
# trainAttribut, trainLabel = traindata()
# compare_Label2, compare_test2, compare_train2=extract_label(trainAttribut, finalLabel_key, trainLabel, finalLabel_value)
# Value_train2, Value_test2 = labelTrans(compare_train2, compare_test2)
# hypothesis2=[0 for _ in range(len(compare_test2))]
# weight2 = [1/len(compare_test2) for _ in range(len(compare_test2))]
#
# error_list2 = bgd(Value_train2, Value_test2, weight2, hypothesis2)


plt.figure()
plt.plot(error_list, color='green', linewidth=2)
