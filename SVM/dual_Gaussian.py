...
for i in range(0,len(C)):
    for j in range(0,len(gamma)):
        w,b,alpha_optimal= dual_gaussian(train_data,C[i],gamma[j])
        svm_sum =0.0
        for m in range(0,alpha_optimal.shape[0]):
            if alpha_optimal[m] != 0.0:
                svm_sum += 1
        print('when C = ',C[i],', gamma = ',gamma[j])
        print('w is ', w)
        print('b is ', b)
        trainerror = errorRate(train_data,b,alpha_optimal,gamma[j])
        print('train error is ', trainerror)
        testerror = errorRate(test_data,b,alpha_optimal,gamma[j])
        print('test error is ', testerror)
        print('the number of support vectors is ', svm_sum)
